...
app.post('/saveQuestion', (req, res) => {
  ...
  // Đọc dữ liệu hiện tại của file JSON (nếu có)
  let data = [];
  if (fs.existsSync('quest_lib.json')) {
    const jsonData = fs.readFileSync('quest_lib.json', 'utf8');
    data = JSON.parse(jsonData);
  }

  // Thêm câu hỏi mới vào mảng dữ liệu
  data.push({
    question: question,
    answers: answers
  });
  ...
  fs.writeFileSync('quest_lib.json', JSON.stringify(data));
